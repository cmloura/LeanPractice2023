    ...
    Nat.recOn a (rfl) (λ(n : Nat) =>
      λ(h1 : ChrisSum n = n * (n+1) / 2) =>
        calc
          ...
          _ = ((n+1) * n /2) + (n+1)*1          := by rw[Nat.mul_one]
          _ = (n+1)*(n/2) + (n+1)*1             := sorry
    ...
    Nat.recOn a (rfl) (λ(n : Nat) =>
      λ(h1 : GrahamSum n = n*n) =>
        calc
          ...
          _ = (n * n) + 2*(n+1) - 1                                := by rw[h1]
          _ = (n * n) + (2*n + 2*1) - 1                            := by rw[Nat.left_distrib]
          _ = (n * n) + (2*n + 2) - 1                              := rfl
          _ = (n * n) + 2*n + 2 - 1                                := by rw[Nat.add_assoc]
          _ = (n * n) + 2*n + 1                                    := rfl
          _ = ((n*n) + 2*n) + 1                                    := by rw[Nat.add_assoc]
          _ = ((n*n) + (2*n)) + 1                                  := rfl
          _ = ((n*n) + (n*2)) + 1                                  := by simp[Nat.mul_comm]
          _ = (n * (n + 2)) + 1                                    := by rw[<-Nat.left_distrib]
          _ = 
          _  = (n+1)*(n+1)                                         := sorry
    )